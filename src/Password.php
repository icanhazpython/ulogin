...
class Password
{
    ...
    private static function BCryptSalt()
    {
        ...
        return $salt_prefix . self::GensaltBlowfish(Utilities::RandomBytes(16));
    }
    ...
    {
        if (Utilities::BeginsWith($salt, '{SSHA}')) {
            $salt = substr($salt, 6);
            if ($salt == '') {
                $salt = Utilities::RandomBytes(24, true);
            }
            return '{SSHA}' . base64_encode(pack('H*', sha1($password . $salt)) . $salt);
        } else {
            if (Utilities::BeginsWith($salt, '{SHA}')) {
                return '{SHA}' . base64_encode(pack('H*', sha1($password)));
            } else {
                if (Utilities::BeginsWith($salt, '{SMD5}')) {
                    $salt = substr($salt, 6);
                    if ($salt == '') {
                        $salt = Utilities::RandomBytes(24, true);
                    }
                    ...
                    if (Utilities::BeginsWith($salt, '{MD5}')) {
                        return '{MD5}' . base64_encode(pack('H*', md5($password)));
                    } else {
                        if (Utilities::BeginsWith($salt, '{CRYPT}')) {
                            $salt = substr($salt, 7);
                            if ($salt == '') {
                            ...
                            if (Utilities::BeginsWith($salt, '{PBKDF2}')) {
                                $hash_bytes = 96;
                                $hash_algo = UL_HMAC_FUNC;
                                $hash_rounds = pow(2, UL_PWD_ROUNDS);
                                $salt = substr($salt, 8);
                                if ($salt == '') {
                                    $salt = Utilities::RandomBytes(16, true);
                                } else {
                                    $parts = explode(':', $salt);
                                    $hash_algo = $parts[0];
                                    $hash_rounds = $parts[1];
                                ...
                                base64_encode(Utilities::pbkdf2(
                                    $hash_algo,
                                    $password,
                                    $salt,
                                    $hash_rounds,
                                ...
                                if (Utilities::BeginsWith($salt, '{BCRYPT}')) {
                                    $salt = substr($salt, 8);
                                }
    ...
    {
        if (Utilities::BeginsWith($hash, '{SSHA}')) {
            $hash = base64_decode(substr($hash, 6));
            return '{SSHA}' . substr($hash, 20);
        } else {
            if (Utilities::BeginsWith($hash, '{SHA}')) {
                return '{SHA}';
            } else {
                if (Utilities::BeginsWith($hash, '{SMD5}')) {
                    $hash = base64_decode(substr($hash, 6));
                    return '{SMD5}' . substr($hash, 16);
                } else {
                    if (Utilities::BeginsWith($hash, '{MD5}')) {
                        return '{MD5}';
                    } else {
                        if (Utilities::BeginsWith($hash, '{CRYPT}')) {
                            $hash = substr($hash, 7);
                            return '{CRYPT}' . substr($hash, 0, 29);
                        } else {
                            if (Utilities::BeginsWith($hash, '{PBKDF2}')) {
                                $parts = explode(':', $hash);
                                array_pop($parts);
