...
class Database
{
    ...
    {
        Utilities::ul_fail('DB error ' . self::ErrorCode() . ': ' . self::ErrorMsg());
    }
        ...
        if ((self::$dbcon == null) || (self::$dbmode != $mode)) {
            ...
            switch ($mode) {
                ...
                case 'session':
                    ...
                    break;
                default:
                    Utilities::ul_fail('Invalid database open mode.');
                    return null;
                    break;
            }

            try {
                self::$dbcon = new PDO(UL_PDO_CON_STRING, $ul_db_user, $ul_db_pwd,
                    array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
            } catch (PDOException $e) {
                Utilities::ul_fail('Cannot open database connection.');
                return null;
            }
    ...
    private static function GetPDOType($str)
    {
        ...
        Utilities::ul_fail('Unknown data type $str.');
        return 0;
    }
    ...
    public static function TableExists($dbuser, $table_name)
    {
        ...
        $stmt = Database::Prepare($dbuser, $query);
        if (false === $stmt) {
            Database::Fail();
        }
