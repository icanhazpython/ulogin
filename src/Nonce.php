...
namespace Ulogin;
use Ulogin\Backend\Auth\PDOBackend\NonceStore;
use \DateTime;
...
class Nonce
{
    private static function StoreVolatile($action, $code, $expire)
    {
        ...
        $newnonce = array(
            'code' => $code,
            'expire' => Utilities::date_seconds_add($now, $expire)->format('c')
        );
    ...
    {
        return NonceStore::Store($action, $code, $expire);
    }
    ...
    {
        return NonceStore::Verify($action, $code);
    }
    ...
    {
        return NonceStore::Exists($action);
    }
...
// ------------------------------------------------
    ...
    {
        $code = Utilities::RandomBytes(16, true);
