...
class Logger
{
    ...
    public static function Clean()
    {
        ...
        if (UL_MAX_LOG_AGE > 0) {
            $max_log_age = Utilities::date_seconds_sub(new DateTime(), UL_MAX_LOG_AGE)->format(UL_DATETIME_FORMAT);
            $stmt = Database::Prepare('log', 'DELETE FROM ul_log WHERE timestamp<?');
        ...
        if (UL_MAX_LOG_RECORDS > 0) {
            $log_num_rows = 0;
            $stmt = Database::Prepare('log', 'SELECT COUNT(*) FROM ul_log');
            if (!Database::BindExec(
                $stmt,
                array(        // output
                    &$log_num_rows,
            ...
            if ($log_num_rows > UL_MAX_LOG_RECORDS) {
                ...
                $stmt = Database::Prepare('log', 'DELETE FROM ul_log ORDER BY timestamp ASC LIMIT ?');
                if (!Database::BindExec(
                    $stmt,
                    null,        // output
                    array(        // input
    ...
    public static function GetFrequencyForUser($username, $action, $window)
    {
        ...
        $time_before_window = Utilities::date_seconds_sub(new DateTime(), $window)->format(UL_DATETIME_FORMAT);
        $stmt = Database::Prepare('log', 'SELECT COUNT(*) FROM ul_log WHERE action=? AND timestamp>? AND user=?');
        if (!Database::BindExec(
            $stmt,
            array(        // output
                &$user_login_attempts,
    ...
    public static function GetFrequencyForIp($ip, $action, $window)
    {
        ...
        $ip_login_attempts = 0;
        $time_before_window = Utilities::date_seconds_sub(new DateTime(), $window)->format(UL_DATETIME_FORMAT);
        $stmt = Database::Prepare('log', 'SELECT COUNT(*) FROM ul_log WHERE action=? AND timestamp>? AND ip=?');
        if (!Database::BindExec(
            $stmt,
            array(        // output
                &$ip_login_attempts,
    ...
    public static function GetUserLastLoginAgo($username)
    {
        ...
        $last_login = '';
        $stmt = Database::Prepare('log',
            "SELECT timestamp FROM ul_log WHERE user=? AND action='auth-success' ORDER BY timestamp DESC LIMIT 1");
        if (!Database::BindExec(
            $stmt,
            array(        // output
                &$last_login,
    ...
    public static function GetIpLastLoginAgo($ip)
    {
        ...
        $last_login = '';
        $stmt = Database::Prepare('log',
            "SELECT timestamp FROM ul_log WHERE ip=? AND action='auth-success' ORDER BY timestamp DESC LIMIT 1");
        if (!Database::BindExec(
            $stmt,
            array(        // output
                &$last_login,
