...
{
	return (
		Database::TableExists('session', 'ul_sessions') &&
		Database::TableExists('log', 'ul_blocked_ips') &&
		Database::TableExists('log', 'ul_log') &&
		Database::TableExists('session', 'ul_nonces')
	);
}
...
function install_checks()
{
	...
	$dscr = 'Class Authenticator available?';
	$explain = 'A basic check to see if one of the core classes is available.';
	if (class_exists('Ulogin\Authenticator'))
		$result = 'OK';
	else
		$result = 'Error';
	...
	if (defined('PDO::ATTR_DRIVER_NAME') && class_exists('Ulogin\Backend\Auth\PDOBackend\Database'))
		$result = 'OK';
	else
		$result = 'Error';
	add_test($dscr, $explain, $result);
...
function generate_keys()
{
	for ($i = 0; $i < 10; ++$i)
	{
		$key = Utilities::RandomBytes(42,  true);
		echo "$key<br>";
	}
}
