...
use Ulogin\SessionManager;
use ULogin\Nonce;
use ULogin\Backend\Auth\DuoLoginBackend;

Utilities::PageInit();

if (!SessionManager::sses_running())
...
$uloginFactorOne = new Authenticator('appAuthFactorOne', 'appLoginFail');

// The second uLogin instance is used to perform second-factor auth.
// We specify the backend we want to use explicitly here.
$uloginFactorTwo = new Authenticator('appAuthFactorTwo', 'appLoginFail', new DuoLoginBackend());
...
if (isAppLoggedIn()){
	...
	if (($action=='login'))
	{
		if (!isset($_SESSION['loginPhase1Success'])) {	// are we authenticating the first factor?
			// Nonce verification
			if (isset($_POST['nonce']) && Nonce::Verify('login', $_POST['nonce'])){
...
?>
	...
	<tr>
		...
		<td>
			<input type="text" id="nonce" name="nonce" value="<?php echo Nonce::Create('login');?>">
		</td>
	</tr>
